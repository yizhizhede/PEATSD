...
# outfile
tmp=${h3:13}
outfile="./tmp/${tmp//\//_}_hv_line_char"
...
for h4 in ${h3}/*; do		# h4 has arrived at the title of algorithm
	...
	algs="$algs,:${h4##*/}:"
	h5="${h4}/hv"		# h5 has arrived the the type of flie
...
echo "a=max(max(R))+1.0e-8;" >> $outfile.m
...
echo "ylabel('1/(a-hv), a=max(hv)+1.0e+8.0');" >> $outfile.m
