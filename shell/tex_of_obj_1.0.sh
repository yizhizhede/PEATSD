...
for alg in $ALG; do
         echo "\\includegraphics[scale=0.22]{Obj-$pro-$(printf '%02d' $obj)-$(printf '%05d' $var)-$alg}" >> $outfile
done
         echo "\\caption{$pro with $obj objectives $var variables.}" >> $outfile
         echo "\\label{fig:chart-of-obj-$pro-$obj-$var}" >> $outfile
