...
for h4 in ${h3}/*; do		# h4 has arrived at the title of algorithm
	...
	algs="$algs ${h4##*/}"
	h5="${h4}/var"		# h5 has arrived the the type of flie
...
for i in $(seq 1 $nalg); do
	...
	echo "X=repmat(1:1:size(M, 1), [1, size(M, 2)]);" >> $outfile.m
	echo "scatter (X, M);" >> $outfile.m
	echo "box on;" >> $outfile.m
	echo "title('${str[ $[ ${i} - 1 ] ]}');" >> $outfile.m
	echo "set(gca,'XTick', [1, size(M, 1)]);" >> $outfile.m
	echo "set(gca,'XTickLabel', [1, size(M, 1)]);" >> $outfile.m
	echo "xlim([1, size(M, 1)]);" >> $outfile.m
	echo "xlabel('Number of variable');" >> $outfile.m
	echo "ylabel('Value of variable');" >> $outfile.m
done
