...
for obj in ${OBJ}; do 			# each objective is related with one table, respectively.
	...
	echo "\\caption{The Average (the standard variance) of HV value obtained by ${ALG// /, } on the problems with ${obj##*0} objectives. The best perfermance is marked by lightgray.}" >> $outfile.tex
...
for h3  in ${h2}/*; do		# h3 has arrived at the number of variables
	...
	C2=$( echo ${h3##*/} | sed 's/^VAR0*\(.*\)$/\1/g')	# The colume 2
...
for alg in $ALG; do
	...
	h5="${h4}/hv"		# h5 has arrived at the type of file
