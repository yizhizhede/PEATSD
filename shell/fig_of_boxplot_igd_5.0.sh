...
for var in ${VAR}; do
	...
	matscript=./tmp/BoxPlotIGD${pro}O$(printf '%02d' $obj)V$(printf '%05d' $var).m
	...
	# echo "print('tmp/Box-IGD-${pro}-$(printf '%02d' $obj)-$(printf '%05d' $var)','-depsc');" >> $matscript
	echo "print('Box-IGD-${pro}-$(printf '%02d' $obj)-$(printf '%05d' $var)','-depsc');" >> $matscript
