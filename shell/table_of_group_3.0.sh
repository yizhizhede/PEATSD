...
echo "\\caption{Grouping of Algorithms}" >> $outfile.tex
...
for var in ${VAR}; do
	...
	echo "if M(1,i) < V" >> $matscript
		echo "fprintf(',%.2f',  M(1,i));" >> $matscript
	echo "else" >> $matscript
		echo "fprintf(',\\\\colorbox{lightgray}{%.2f}',  M(1,i));" >> $matscript
	echo "end" >> $matscript
	echo "end" >> $matscript
