...
# outfile
tmp=${h3:13}
outfile="./tmp/${tmp//\//_}_igd_line_char"
...
for h4 in ${h3}/*; do		# h4 has arrived at the title of algorithm
	...
	algs="$algs,:${h4##*/}:"
	h5="${h4}/igd"		# h5 has arrived the the type of flie
...
echo "a=min(min(R))-0.001;" >> $outfile.m
echo "Y=1.0 ./ (a-R);" >> $outfile.m
...
echo "ylabel('1/(a-igd), a=min(igd) - 1.0e-3.0');" >> $outfile.m
echo "legend(${legend});" >> $outfile.m
