...
for obj in ${OBJ}; do 			# each objective is related with one table, respectively.
	...
	echo "\\caption{The Average of running-time(s) consumed by ${ALG// /,} on the problems with ${obj##*0} objectives. The minimum in each row is marked by lightgray.}" >> $outfile.tex
...
for alg in $ALG; do
	...
	h5="${h4}/time"		# h5 has arrived at the type of file
	...
	echo "if i == I(1)" >> $tmp
	echo "printf('\\\\colorbox{lightgray}{%.0f},', A(i));" >> $tmp
	echo "else" >> $tmp
	echo "printf('%.0f,', A(i));" >> $tmp
	echo "end" >> $tmp
	echo "end" >> $tmp
