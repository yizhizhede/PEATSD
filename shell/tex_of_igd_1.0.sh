...
for pro in $PRO; do
	...
	echo "\\includegraphics[width=0.45\\textwidth]{IGD-$pro-$(printf '%02d' $obj)-$(printf '%05d' $var)}" >> $outfile
	echo "\\caption{$pro with $obj objectives $var decision variables.}" >> $outfile
	echo "\\label{fig:chart-of-igd-curve-$pro-$obj-$var}" >> $outfile
