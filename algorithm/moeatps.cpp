...
// Main framewok of MOEA/TPS
Population_t* moeatps (Problem_t *problem) {
...
// TPS optimize
static void TPS_optimize_convergence (Population_t* pop,  TSS_Pop_t* tssPop, int *I);
static void TPS_optimize_diversity (Population_t* pop,  TSS_Pop_t* tssPop);
...
static void TSS_reproduce_CSO (Population_t* pop, TSS_Pop_t* popParent, TSS_Pop_t* popOffspring) {
	...
	// TPS optimize
	if (rand () % 10 < 1) {
		TPS_optimize_diversity (pop, popParent);
	} else {
		TPS_optimize_convergence (pop, popParent, I);
	}
...
// TPS optimize
static void TPS_optimize_one (Population_t* pop);
...
/*************	TPS  *****************************************************************************************/
/*************************************************************************************************************/
// aggregation function for TPS
static double g_sum (double *f, double *lambda, int len) {
         return sum (f, len);
}

static double g_p2l (double *f, double *lambda, int len) {
 	return distance_p2l (f, lambda, len);
}

// TPS Optimize
static void TPS_optimize (Population_t* pop, int j, int i, double (*g_func)(double *f, double *w, int M), double *gW) {
	int	numVar  = pop->var->colDim;
	int	numObj	= pop->obj->colDim; 
	int	k;

	// parameter of TPS
	double 	x[3*numVar+10]; 
	double 	y[3*numObj+10]; 
	double*	bx = pop->var->elements + j*numVar; 
	double* by = pop->obj->elements + j*numObj;

	// copy (bx, by) to (x, y)
	memcpy (x, bx, numVar*sizeof (double));
	memcpy (y, by, numObj*sizeof (double));

	// init
	tps_init (x, numVar, y, numObj, i, g_func, gW);

	// exec
	tps_exec (x, numVar, y, numObj, i, 1000);

	// update the best one
	if (g_func == g_sum) {
		for (k=0; k<3; k++) if (1 == isDominate (y+k*numObj, by, numObj)) {
			memcpy (bx, x+k*numVar, numVar*sizeof (double));
			memcpy (by, y+k*numObj, numObj*sizeof (double));
		}
	} else {
		memcpy (bx, x+1*numVar, numVar*sizeof (double));
		memcpy (by, y+1*numObj, numObj*sizeof (double));
	}
}


// TPS optimize for one solution
static void TPS_optimize_one (Population_t* pop) {
	int	numVar  = pop->var->colDim;
	int	i, j = 0;

	for (i=0, j=0; i<numVar; i++) {
		TPS_optimize (pop, j, i, g_sum, NULL);
	}
}

// TPS optimize for convergence 
static void TPS_optimize_convergence (Population_t* pop,  TSS_Pop_t* tssPop, int *I) {
	Population_t*	tmpPop 	= NULL;
	int		popSize = tssPop->popSize;
	int		numVar  = tssPop->numVar;
	int		numObj  = tssPop->numObj;
	int		i, j, k;
	...
	// allocate memory
	tmpPop = (Population_t*) malloc (sizeof (Population_t));
	tmpPop->var = Matrix_new (1, numVar);
	tmpPop->obj = Matrix_new (1, numObj);
	tmpPop->biased = pop->biased;
	memcpy (tmpPop->var->elements, tssPop->var+j*numVar, numVar*sizeof (double));
	memcpy (tmpPop->obj->elements, tssPop->obj+j*numObj, numObj*sizeof (double));

	// TPS
	TPS_optimize (tmpPop, 0, i, g_sum, NULL);

	// update the j-th individual
	if (1 == isDominate (tmpPop->obj->elements, tssPop->obj+j*numObj, numObj)) {
		memcpy (tssPop->var+j*numVar, tmpPop->var->elements, numVar*sizeof (double));
		memcpy (tssPop->obj+j*numObj, tmpPop->obj->elements, numObj*sizeof (double));
	}

	// free
	Matrix_free (&tmpPop->obj);
	Matrix_free (&tmpPop->var);
	free (tmpPop);
}

// TPS optimize for diversity 
static 	Matrix_t*	TPS_Z = NULL;
//
static void TPS_optimize_diversity (Population_t* pop,  TSS_Pop_t* tssPop) {
	Population_t*	tmpPop 	= NULL;
	int		popSize = tssPop->popSize;
	...
	int		i, j, k;
	double		z[numObj+10];
	int		queue[2*numVar+10], n = 0;

	// set reference vector
	if (NULL == TPS_Z) {
		TPS_Z = Population_reference (Problem_get());		// reference vector of objective
	}

	// select a reference vector randomly
	k = rand () % (TPS_Z->rowDim); 
	memcpy (z, TPS_Z->elements+k*numObj, numObj*sizeof (double));

	// index of variable
	for (k=0, n=0; k<numVar; k++) if (1 == pop->CS[k] || 1 == pop->CV[k]) {
		queue[n++] = k;	
	}
	if (n > 0) {
		i = queue[rand () % n];
	...
	// TPS
	TPS_optimize (tmpPop, 0, i, g_p2l, z);

	// update the j-th individual
	if (1 != isDominate (tssPop->obj+j*numObj, tmpPop->obj->elements, numObj)) {
		memcpy (tssPop->var+j*numVar, tmpPop->var->elements, numVar*sizeof (double));
		memcpy (tssPop->obj+j*numObj, tmpPop->obj->elements, numObj*sizeof (double));
	}
